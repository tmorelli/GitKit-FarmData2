...
let TreeViewWelcomeWidget = class TreeViewWelcomeWidget extends tree_widget_1.TreeWidget {
    ...
    renderViewWelcome() {
        return ((0, jsx_runtime_1.jsxs)("div", { className: 'theia-WelcomeView', children: [...this.viewWelcomeNodes] }));
    }
        ...
        for (const { content, enablement } of items) {
            ...
            for (let line of lines) {
                ...
                if (linkedTextItems.length === 1 && typeof linkedTextItems[0] !== 'string') {
                    ...
                    this.viewWelcomeNodes.push((0, jsx_runtime_1.jsxs)("p", { children: [...linkedTextItems.flatMap(renderNode)] }, `p-${this.viewWelcomeNodes.length}`));
                }
            }
        }
    }
    renderButtonNode(node, lineKey, enablement) {
        return ((0, jsx_runtime_1.jsx)("div", { className: 'theia-WelcomeViewButtonWrapper', children: (0, jsx_runtime_1.jsx)("button", { title: node.title, className: 'theia-button theia-WelcomeViewButton', disabled: !this.isEnabledClick(enablement), onClick: e => this.openLinkOrCommand(e, node.href), children: this.renderLabelWithIcons(node.label) }) }, `line-${lineKey}`));
    }
    renderTextNode(node, textKey) {
        return (0, jsx_runtime_1.jsx)("span", { children: this.renderLabelWithIcons(node) }, `text-${textKey}`);
    }
    renderLinkNode(node, linkKey, enablement) {
        return ((0, jsx_runtime_1.jsx)("a", { className: this.getLinkClassName(node.href, enablement), title: node.title || '', onClick: e => this.openLinkOrCommand(e, node.href), children: this.renderLabelWithIcons(node.label) }, `link-${linkKey}`));
    }
    renderLabelWithIcons(label) {
        return this.labelParser.parse(label).map((segment, index) => label_parser_1.LabelIcon.is(segment)
            ? (0, jsx_runtime_1.jsx)("span", { className: (0, widgets_1.codicon)(segment.name) }, index)
            : (0, jsx_runtime_1.jsx)("span", { children: segment }, index));
    }
