...
class WorkspaceMainImpl {
    constructor(rpc, container) {
        ...
        this.toDispose.push(this.workspaceTrustService.onDidChangeWorkspaceTrust(trust => {
    ...
    async $resolveEncoding(resource, options) {
        let encoding;
        if (resource) {
            encoding = await this.encodingRegistry.getEncodingForResource(uri_1.default.fromComponents(resource), options?.encoding);
        }
        else {
            encoding = options?.encoding ?? encodings_1.UTF8;
        }
    ...
    async getPreferredReadEncoding(uri, options, detectedEncoding) {
        ...
        if (uri) {
            ...
            encoding = preferredEncoding ?? encodings_1.UTF8;
        }
        return encoding;
    }
}
