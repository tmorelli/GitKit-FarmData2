...
export class InputBox extends Widget {
    ...
    _showMessage() {
        ...
        if (this.message.type === 3 /* MessageType.ERROR */) {
            alertText = nls.localize(17, "Error: {0}", this.message.content);
        }
        else if (this.message.type === 2 /* MessageType.WARNING */) {
            alertText = nls.localize(18, "Warning: {0}", this.message.content);
        }
        else {
            alertText = nls.localize(19, "Info: {0}", this.message.content);
        }
...
export class HistoryInputBox extends InputBox {
    constructor(container, contextViewProvider, options) {
        const NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_NO_PARENS = nls.localize(20, ' or {0} for history', `\u21C5`);



        const NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_IN_PARENS = nls.localize(21, ' ({0} for history)', `\u21C5`);
    ...
    showNextValue() {
        ...
        this.value = next ?? '';
        aria.status(this.value ? this.value : nls.localize(22, "Cleared Input"));
    }
    showPreviousValue() {
        if (!this.history.has(this.value)) {
            this.addToHistory();
