...
export class AbstractGotoLineQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
    ...
    provideWithoutTextEditor(picker) {
        const label = localize(1467, "Open a text editor first to go to a line or an offset.");
        picker.items = [{ label }];
    ...
    provideWithTextEditor(context, picker, token) {
        ...
        const offsetButton = {
            ...
            tooltip: localize(1468, "Toggle Zero-Based Offset"),
    ...
    parsePosition(editor, value) {
        ...
        if (!model) {
            return {
                label: localize(1469, "Open a text editor first to go to a line or an offset.")
            };
        }
        ...
        if (value.startsWith(':')) {
            ...
            if (isNaN(offset)) {
                ...
                return {
                    ...
                    label: this.useZeroBasedOffset ?
                        localize(1470, "Type a character position to go to (from 0 to {0}).", maxOffset - 1) :
                        localize(1471, "Type a character position to go to (from 1 to {0}).", maxOffset)
                };
            }
            else {
                ...
                return {
                    ...pos,
                    inOffsetMode: true,
                    label: localize(1472, "Press 'Enter' to go to line {0} at column {1}.", pos.lineNumber, visibleColumn)
                };
            }
        }
        else {
            ...
            if (parts.length < 1 || isNaN(lineNumber)) {
                return {
                    label: localize(1473, "Type a line number to go to (from 1 to {0}).", maxLine)
                };
            }
            ...
            if (parts.length < 2 || isNaN(column)) {
                return {
                    ...
                    label: parts.length < 2 ?
                        localize(1474, "Press 'Enter' to go to line {0} or enter colon : to add a column number.", lineNumber) :
                        localize(1475, "Press 'Enter' to go to line {0} or enter a column number (from 1 to {1}).", lineNumber, maxColumn)
                };
            }
            ...
            return {
                lineNumber,
                column: realColumn,
                label: localize(1476, "Press 'Enter' to go to line {0} at column {1}.", lineNumber, column)
            };
        }
    }
}
