    ...
    openLinkOccurrence(occurrence, openToSide, fromUserGesture = false) {
        ...
        link.resolve(CancellationToken.None).then(uri => {
            ...
            if (messageOrError === 'invalid') {
                this.notificationService.warn(nls.localize(1409, 'Failed to open this link because it is not well-formed: {0}', link.url.toString()));
            }
            else if (messageOrError === 'missing') {
                this.notificationService.warn(nls.localize(1410, 'Failed to open this link because its target is missing.'));
            }
            else {
                onUnexpectedError(err);
            }
...
function getHoverMessage(link, useMetaKey) {
    ...
    const label = link.tooltip
        ? link.tooltip
        : executeCmd
            ? nls.localize(1411, 'Execute command')
            : nls.localize(1412, 'Follow link');
    const kb = useMetaKey
        ? platform.isMacintosh
            ? nls.localize(1413, "cmd + click")
            : nls.localize(1414, "ctrl + click")
        : platform.isMacintosh
            ? nls.localize(1415, "option + click")
            : nls.localize(1416, "alt + click");
    if (link.url) {
        let nativeLabel = '';
        if (/^command:/i.test(link.url.toString())) {
            ...
            if (match) {
                ...
                nativeLabel = nls.localize(1417, "Execute command {0}", commandId);
            }
        }
...
class OpenLinkAction extends EditorAction {
    constructor() {
        super({
            ...
            label: nls.localize2(1418, "Open Link"),
