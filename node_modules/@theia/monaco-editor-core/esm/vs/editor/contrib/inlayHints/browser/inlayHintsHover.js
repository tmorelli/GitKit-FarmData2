    ...
    computeAsync(anchor, _lineDecorations, source, token) {
        ...
        return new AsyncIterableProducer(async (executor) => {
            ...
            if (isNonEmptyArray(part.item.hint.textEdits)) {
                executor.emitOne(new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(localize(1289, "Double-click to insert"))], false, 10001));
            }
            ...
            if (part.part.location || part.part.command) {
                ...
                const kb = useMetaKey
                    ? platform.isMacintosh
                        ? localize(1290, "cmd + click")
                        : localize(1291, "ctrl + click")
                    : platform.isMacintosh
                        ? localize(1292, "option + click")
                        : localize(1293, "alt + click");
                if (part.part.location && part.part.command) {
                    linkHint = new MarkdownString().appendText(localize(1294, 'Go to Definition ({0}), right click for more', kb));
                }
                else if (part.part.location) {
                    linkHint = new MarkdownString().appendText(localize(1295, 'Go to Definition ({0})', kb));
                }
                else if (part.part.command) {
                    linkHint = new MarkdownString(`[${localize(1296, "Execute Command")}](${asCommandLink(part.part.command)} "${part.part.command.title}") (${kb})`, { isTrusted: true });
                }
                if (linkHint) {
