...
let AbstractCommandsQuickAccessProvider = class AbstractCommandsQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    async _getPicks(filter, _disposables, token, runOptions) {
        ...
        for (let i = 0; i < filteredCommandPicks.length; i++) {
            ...
            if (i === 0 && isInHistory) {
                commandPicks.push({ type: 'separator', label: localize(1905, "recently used") });
                addOtherSeparator = true;
            }
            if (addSuggestedSeparator && commandPick.tfIdfScore !== undefined) {
                commandPicks.push({ type: 'separator', label: localize(1906, "similar commands") });
            ...
            if (addCommonlyUsedSeparator && commandPick.tfIdfScore === undefined && !isInHistory && this.options.suggestedCommandIds?.has(commandPick.commandId)) {
                commandPicks.push({ type: 'separator', label: localize(1907, "commonly used") });
                addOtherSeparator = true;
            ...
            if (addOtherSeparator && commandPick.tfIdfScore === undefined && !isInHistory && !this.options.suggestedCommandIds?.has(commandPick.commandId)) {
                commandPicks.push({ type: 'separator', label: localize(1908, "other commands") });
        ...
        return {
            ...
            additionalPicks: (async () => {
                ...
                if (addSuggestedSeparator && commandPicks[0]?.type !== 'separator') {
                    commandPicks.unshift({ type: 'separator', label: localize(1909, "similar commands") });
                }
                return commandPicks;
            })()
        };
    ...
    toCommandPick(commandPick, runOptions, isRecentlyUsed = false) {
        ...
        const ariaLabel = keybinding ?
            localize(1910, "{0}, {1}", commandPick.label, keybinding.getAriaLabel()) :
            commandPick.label;
        ...
        const buttons = isRecentlyUsed ? [
            ...existingButtons,
            {
                ...
                tooltip: localize(1911, "Remove from Recently Used")
            }
        ] : commandPick.buttons;
        return {
            ...commandPick,
            ...
            accept: async () => {
                ...
                try {
                    ...
                    if (!isCancellationError(error)) {
                        this.dialogService.error(localize(1912, "Command '{0}' resulted in an error", commandPick.label), toErrorMessage(error));
                    }
                }
            },
